...
def load_traindata(filename):
    ...
    while line:
        ...
        input_seq.append(np.asarray(input_data, dtype=np.float64))
        line = f.readline()

    f.close()

    input_seqs = []
    input_seqs.append(np.asarray(input_seq, dtype=np.float64))
    
    for i in range(2, 102):
        new_input_seq = map(operator.add, input_seq, np.random.standard_normal(len(input_seq)))
        input_seqs.append(np.asarray(new_input_seq, dtype=np.float64))
    ...
    for input_seq in input_seqs:  
        ...
        for input_data in input_seq:
            if count > 1 and count <= n_seq:
                target_seq.append(np.asarray(input_data, dtype=np.float64))
            count = count + 1 
        target_seq.append(np.asarray(input_data, dtype=np.float64))  
        target_seqs.append(np.asarray(target_seq, np.float64))
...
def test_real():
    ...
    input_seqs = load_traindata('trajectory.txt')
    target_seqs = get_targets(input_seqs)
    
    input_seqs.pop()
    target_seqs.pop()

    n_u = 2 # input vector size (not time at this point)
    n_h = 100 # hidden vector size
